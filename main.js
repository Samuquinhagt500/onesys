...
// CONFIGURAÇÃO DO BANCO (Mantenha sua string de conexão)
...
function createWindow() {
    win = new BrowserWindow({ width: 1300, height: 950, title: 'OneSys Construtora', webPreferences: { nodeIntegration: true, contextIsolation: false } });
...
ipcMain.handle('get-system-users', async () => await dbQuery("SELECT id, username, cargo, reset_req FROM users ORDER BY username ASC"));
...
ipcMain.handle('get-project-materials', async (event, obraId) => {
    return await dbQuery(`SELECT pm.*, p.nome as nome_material, p.categoria, p.valor_unitario FROM project_materials pm LEFT JOIN products p ON pm.produto_id = p.id WHERE pm.obra_id = ${obraId}`);
});
...
ipcMain.handle('save-finance-entry', async (event, gasto) => {
    try {
        if (!gasto.id) {
            await dbQuery(`INSERT INTO finance_entries (tipo, descricao, valor, categoria_id, data_vencimento, status, data_pagamento) VALUES ($1, $2, $3, $4, $5, $6, $7)`, 
...
ipcMain.handle('add-employee-full', async (e, d) => { 
    ...
    await client.query("INSERT INTO employee_history (employee_id, tipo, descricao, data_evento) VALUES ($1, 'ADMISSAO', $2, NOW())", [novoId, `Admitido como ${d.cargo}`]);
...
ipcMain.handle('get-notifications', async () => {
    try {
        const alertasRH = await dbQuery(`
            SELECT nome, aso_vencimento, ferias_inicio, 'RH' as origem 
            FROM employees 
...
// ==========================================================
// --- SCRIPT DE CORREÇÃO AUTOMÁTICA DO DB (V70) ---
// ==========================================================
setTimeout(async () => {
    console.log(">>> VERIFICANDO E ATUALIZANDO BANCO DE DADOS...");
    try {
        await dbQuery(`CREATE TABLE IF NOT EXISTS employee_documents (id SERIAL PRIMARY KEY, employee_id INT, nome_arquivo TEXT, conteudo TEXT, tipo_arquivo TEXT, data_upload TIMESTAMP DEFAULT NOW());`);
        await dbQuery(`CREATE TABLE IF NOT EXISTS employee_history (id SERIAL PRIMARY KEY, employee_id INT, tipo TEXT, descricao TEXT, data_evento TIMESTAMP DEFAULT NOW());`);
        ...
        console.log("✅ BANCO DE DADOS VERIFICADO E ATUALIZADO (V70).");
    } catch (err) { console.error("Erro DB Auto-Fix:", err.message); }
}, 3000);