...
async function configurarBanco() {
    ...
    try {
        await client.connect();
        console.log("--- SINCRONIZANDO BANCO (V33) ---");
        ...
        await client.query(`CREATE TABLE IF NOT EXISTS project_materials (id SERIAL PRIMARY KEY, project_id INTEGER REFERENCES projects(id), nome_material TEXT, quantidade INTEGER, valor_unitario NUMERIC, data_envio TIMESTAMP DEFAULT NOW())`);
        ...
        await client.end();
        console.log("✅ BANCO DE DADOS PRONTO (V33).");
...
ipcMain.handle('get-projects', async () => { 
    let sql = `SELECT p.*, c.nome as cliente_nome, u1.username as engenheiro_nome, u2.username as mestre_nome FROM projects p LEFT JOIN clients c ON p.cliente_id = c.id LEFT JOIN users u1 ON p.engenheiro_id = u1.id LEFT JOIN users u2 ON p.mestre_id = u2.id`; 
...
// MATERIAIS DA OBRA
ipcMain.handle('get-project-materials', async (e, obraId) => {
    return await dbQuery("SELECT * FROM project_materials WHERE project_id = $1 ORDER BY data_envio DESC", [obraId]);
});
...
ipcMain.handle('transfer-product', async (e, d) => {
    const prod = await dbQuery("SELECT * FROM products WHERE id=$1", [d.prodId]);
    if (prod.length === 0 || prod[0].quantidade < d.qtd) return { sucesso: false, msg: "Estoque insuficiente!" };
    await dbQuery("UPDATE products SET quantidade = quantidade - $1 WHERE id=$2", [d.qtd, d.prodId]);
    const checkObra = await dbQuery("SELECT * FROM project_materials WHERE project_id=$1 AND nome_material=$2", [d.obraId, prod[0].nome]);
    if (checkObra.length > 0) { await dbQuery("UPDATE project_materials SET quantidade = quantidade + $1 WHERE id=$2", [d.qtd, checkObra[0].id]); } 
    else { await dbQuery("INSERT INTO project_materials (project_id, nome_material, quantidade, valor_unitario) VALUES ($1, $2, $3, $4)", [d.obraId, prod[0].nome, d.qtd, prod[0].valor_unitario]); }
    return { sucesso: true };
});
