/* ARQUIVO: main.js (V31 - ALMOXARIFADO COMPLETO + CLIENTES + RH) */
...
async function configurarBanco() {
    ...
    try {
        await client.connect();
        console.log("--- SINCRONIZANDO BANCO (V31) ---");
        ...
        await client.end();
        console.log("✅ BANCO DE DADOS PRONTO (V31).");
    } catch (err) { console.log("Erro Banco:", err.message); }
}
configurarBanco();

function createWindow() {
    win = new BrowserWindow({ width: 1300, height: 950, title: 'OneSys Construtora v31', webPreferences: { nodeIntegration: true, contextIsolation: false } });
...
ipcMain.handle('save-product', async (e, d) => {
    if (d.id) {
        // Editar
        await dbQuery("UPDATE products SET nome=$1, categoria=$2, quantidade=$3, valor_unitario=$4 WHERE id=$5", [d.nome, d.cat, d.qtd, d.val, d.id]);
    } else {
        // Criar
        await dbQuery("INSERT INTO products (nome, categoria, quantidade, valor_unitario) VALUES ($1, $2, $3, $4)", [d.nome, d.cat, d.qtd, d.val]);
    }
    return true;
});
